...
- **Manter conectado por agenda** — no cadastro de cada host há uma faixa de **dias da semana + horário** (ex.: *Seg a Sex, 08:00–18:00*, ou *Sáb, 22:00–02:00* para uma janela de manutenção). Dentro dessa faixa o app **abre a conexão sozinho** e a aba **não pode ser fechada**: no lugar do "×" aparece um cadeado que diz até quando. Fora da faixa nada muda — a aba continua aberta e volta a ser fechável. **Nada é derrubado no fim do período**: interromper um comando em execução seria pior que deixar uma aba a mais na tela. Vale para qualquer protocolo (SSH, Telnet, RDP, VNC, página web). Trava **uma** aba por host — a promessa é que exista uma conexão aberta, e as abas extras que você abrir do mesmo host continuam fechando normalmente. Para 24 horas contínuas use **00:00–24:00**: `23:59` como fim exclui o próprio minuto e deixaria um buraco de 60 s por dia. Se a conexão cair durante o período, o app tenta de novo — no máximo uma vez por minuto por host, e a aba morta é substituída em vez de acumulada. A faixa que **atravessa a meia-noite** pertence ao dia em que começa: *sábado 22:00–02:00* cobre a noite de sábado e a madrugada de domingo, e não a noite de domingo. O horário é o **relógio local** desta máquina, e a agenda vai no backup `.xml`.
...
public/agenda.js   dias da semana + faixa de horário: validação e "está na janela?"
