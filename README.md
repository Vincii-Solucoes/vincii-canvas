...
- **Manter conectado por agenda** — no cadastro de cada host há uma **faixa de horário** que vale **todo dia** (ex.: *08:00–18:00*, ou *22:00–02:00* para uma janela de manutenção da madrugada). Dentro dessa faixa o app **abre a conexão sozinho** e a aba **não pode ser fechada**: no lugar do "×" aparece um cadeado que diz até quando. Fora da faixa nada muda — a aba continua aberta e volta a ser fechável. **Nada é derrubado no fim do período**: interromper um comando em execução seria pior que deixar uma aba a mais na tela. Vale para qualquer protocolo (SSH, Telnet, RDP, VNC, página web). Trava **uma** aba por host — a promessa é que exista uma conexão aberta, e as abas extras que você abrir do mesmo host continuam fechando normalmente. Para 24 horas contínuas marque **"até o fim do dia"**: `23:59` como fim exclui o próprio minuto e deixaria um buraco de 60 s por dia. Se a conexão cair durante o período, o app tenta de novo — no máximo uma vez por minuto por host, e a aba morta é substituída em vez de acumulada. A faixa que **atravessa a meia-noite** simplesmente emenda no dia seguinte: *22:00–02:00* cobre toda noite das 22h às 2h. O horário é o **relógio local** desta máquina, e a agenda vai no backup `.xml`.
  - Um backup `.xml` de terceiro pode trazer agendas, e agenda é o único campo do arquivo que faz o app **agir sozinho** — por isso a confirmação de importação lista os hosts agendados e os horários antes de aplicar.
  - **Soltar uma aba agendada** funciona normalmente, e **fechar essa janela devolve a sessão para a aba do Terminal** em vez de encerrá-la — num terminal ela volta viva, com o mesmo diretório e o mesmo processo rodando. Para isso a janela principal precisa estar aberta: ela é quem cuida da agenda.
- **Soltar aba em janela própria** — o **↗** ao lado do nome de cada aba do Terminal move aquela sessão para uma **janela separada**, que abre só com ela: sem barra de abas, sem barra lateral, a sessão ocupando a janela inteira. Serve para deixar um `tail -f` num monitor e trabalhar no outro, ou comparar dois hosts lado a lado. Num **terminal** (SSH, Telnet ou local) a sessão é **transferida viva**: a conexão mora no servidor do app, não no socket da janela, então o shell continua exatamente onde estava — mesmo diretório, mesmas variáveis de ambiente, mesmo processo rodando, e a tela é reexibida do começo. Em **RDP, VNC e páginas web** a janela nova **reconecta** (esses clientes rodam dentro da janela, não dá para transferi-los) — na prática não se percebe, porque não há estado de shell a perder. **Fechar a janela solta encerra aquela conexão**; se a janela morrer por acidente, a sessão fica guardada por 5 minutos antes de o servidor derrubá-la. Dentro da janela solta continuam disponíveis os **Favoritos**, as **Variáveis** e o painel de IA daquela sessão.
...
- **O que o modelo de ameaça NÃO cobre:** o app confia em qualquer processo local que fale HTTP com ele. Requisição sem cabeçalho `Origin` (curl, script, outro app) é aceita de propósito — a premissa é que um processo rodando como você já tem os seus privilégios. A consequência a ter em mente numa máquina **compartilhada com outros usuários**: eles alcançam `127.0.0.1` também, e daí leem o token do processo na página servida e conseguem abrir terminal nos seus hosts com as credenciais salvas, ou assumir uma sessão órfã. `data.json` continua com permissão 600 (a senha em si não é lida), mas isso não impede o uso dela pelo app. Não use o app logado numa máquina que outras pessoas acessam ao mesmo tempo.
...
lib/termsessions.js  registro de sessões: a conexão sobrevive à janela que a exibe
lib/presenca.js    qual janela mostra qual host (coordenação da agenda)
lib/credenciais.js O ÚNICO ponto que resolve a credencial de um host
lib/cofres/        um adaptador por produto de cofre (nativo, …)
lib/cofresegredos.js  chaves de API dos cofres, fora do data.json e do backup
