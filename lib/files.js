...
const local = {
  ...
  async readText(arquivo) {
    ...
    return await fsp.readFile(arquivo, 'utf8');
  },
  ...
  return {
    ...
    async readText(arquivo) {
      ...
      await new Promise((res, rej) => {
        const s = sftp.createReadStream(arquivo);
        s.on('data', (d) => pedacos.push(d));
...
async function abrirFtp(host) {
  ...
  const base = {
    ...
    password: (host.auth && host.auth.password) || 'anonymous@',
  };
  let seguro = false;
  try {
    if (modo === 'no') {
    ...
    if (modo === 'auto') {
      ...
      return montaFtp(c2, false);
    }
    try { client.close(); } catch {}
    throw new Error(humanizaFtp(err, host));
  }
  return montaFtp(client, seguro);
}

function montaFtp(client, seguro) {
  let atual = '/';
  return {
    tipo: 'ftp',
    seguro,
