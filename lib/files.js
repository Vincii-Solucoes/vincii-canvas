...
async function abrirFtp(host) {
  ...
  try {
    ...
    // A credencial só é entregue ao `montaFtp` quando a conexão VINGA — e é o
    // close() dele que a devolve. Em toda saída por erro, quem devolve é este
    // bloco: sem isto, uma senha de cofre num servidor FTP fora do ar ficava
    // presa no cadastro de redação para sempre, com o valor em texto claro na
    // memória do processo e a contagem daquele segredo nunca zerando.
    if (modo === 'auto') {
      ...
      try {
        ...
        try { c2.close(); } catch {}
        cred.dispose();
        throw new Error(humanizaFtp(err2, host));
      }
