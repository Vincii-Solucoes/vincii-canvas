...
// Em lote: um save só, em vez de N agendamentos.
function removeMany(ids) {
  const alvo = new Set((Array.isArray(ids) ? ids : []).map(String));
  const antes = entries.length;
  for (let i = entries.length - 1; i >= 0; i--) if (alvo.has(String(entries[i].id))) entries.splice(i, 1);
  const removidos = antes - entries.length;
  if (removidos) scheduleSave();
  return removidos;
}

module.exports = { add, list, clear, remove, removeMany, localIp, flush };
