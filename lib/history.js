...
let idSeq = 0;
let saveTimer = null;
let sujo = false; // há alteração ainda não persistida?
...
function load() {
  try {
    ...
    // Corrompido: preserva uma cópia antes de começar vazio, como o store.js já
    // faz com o data.json. Apagar em silêncio a trilha de auditoria (que registra
    // comandos de humano e de IA) é perda irreversível e invisível.
    try {
      const backup = FILE + '.corrompido-' + Date.now();
      fs.copyFileSync(FILE, backup);
      console.warn(`[history] history.json inválido; backup salvo em ${backup}. Começando vazio.`);
    } catch {}
    return [];
  }
}
...
function flush() {
  try {
    gravarAtomico(FILE, JSON.stringify(entries), 0o600);
    sujo = false;
  } catch {
...
// Fechar a janela logo após um comando descartava esse comando: o flush é
// adiado 400 ms e o timer é unref(). Ao sair, grava o que estiver pendente.
// (Handler síncrono: só flush() síncrono funciona em 'exit'.)
process.on('exit', () => { if (sujo) flush(); });
