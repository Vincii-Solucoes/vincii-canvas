...
// ---------- relatório de TCP ping ----------

function normalizarDadosTcpping(b) {
  if (!b || typeof b !== 'object') return null;
  const host = String(b.host || '').trim();
  if (!host || host.length > 255 || !/^[a-zA-Z0-9._:-]+$/.test(host)) return null;
  const porta = Math.round(Number(b.porta));
  if (!Number.isInteger(porta) || porta < 1 || porta > 65535) return null;
  const num = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : null);
  return {
    host, porta,
    inicio: num(b.inicio) || Date.now(), fim: num(b.fim) || Date.now(),
    total: Math.max(0, Math.round(num(b.total) || 0)),
    perdidos: Math.max(0, Math.round(num(b.perdidos) || 0)),
    media: num(b.media), melhor: num(b.melhor), pior: num(b.pior),
  };
}

function htmlRelatorioTcpping(d) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .card { flex: 1; border: 1px solid #d8dee6; border-radius: 8px; padding: 12px 10px; text-align: center; }
    .card b { display: block; font-size: 19px; font-family: 'SF Mono', Menlo, Consolas, monospace; } .card.ruim b { color: #c62828; }
    .card span { color: #667; font-size: 10.5px; text-transform: uppercase; letter-spacing: .4px; }
    .rodape { margin-top: 28px; padding-top: 10px; border-top: 1px solid #e3e8ee; color: #778; font-size: 11px; }
  </style></head><body>
    <div class="topo"><h1>Relatório de TCP ping</h1><span class="marca">Vincii Canvas</span></div>
    <div class="host">${escapa(d.host)}:${d.porta}</div>
    <div class="periodo">${escapa(fmtQuando(d.inicio))} &rarr; ${escapa(fmtQuando(d.fim))} &nbsp;·&nbsp; duração ${escapa(fmtDur(d.fim - d.inicio))}</div>
    <div class="cards">
      <div class="card"><b>${d.total}</b><span>tentativas</span></div>
      <div class="card${d.perdidos ? ' ruim' : ''}"><b>${d.perdidos}</b><span>sem resposta</span></div>
      <div class="card${perda ? ' ruim' : ''}"><b>${perda}%</b><span>falha</span></div>
      <div class="card"><b>${escapa(fmtMs(d.melhor))}</b><span>conexão mín</span></div>
      <div class="card"><b>${escapa(fmtMs(d.media))}</b><span>conexão média</span></div>
      <div class="card"><b>${escapa(fmtMs(d.pior))}</b><span>conexão máx</span></div>
    </div>
    <div class="rodape">Gerado pelo Vincii Canvas em ${escapa(fmtQuando(Date.now()))} · handshake TCP a cada 1 s até a porta ${d.porta}</div>
  </body></html>`;
}
...
module.exports = {
  ...
  normalizarDadosTcpping, htmlRelatorioTcpping,
