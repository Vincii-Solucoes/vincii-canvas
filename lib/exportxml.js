...
function buildXml(data, opts = {}) {
  ...
  for (const h of d.hosts || []) {
    ...
    lines.push(`    <host${attrs({ name: h.name, host: h.host, port: h.port, username: h.username, protocol: h.protocol || 'ssh', ftps: h.protocol === 'ftp' ? (h.ftps || 'auto') : undefined, rdpDomain: h.protocol === 'rdp' ? h.rdpDomain : undefined, group: h.group, icon: h.icon, color: h.color })}>`);
