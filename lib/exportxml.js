...
function escText(s) {
  // \r cru em conteúdo de texto some no parse (o XML normaliza quebras de
  // linha): valor de variável com CR mudava no round-trip. Como referência
  // numérica ele volta igual. Depois do '&', senão o '&' de '&#13;' escapa.
  return cleanChars(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\r/g, '&#13;');
}
...
function buildXml(data, opts = {}) {
  ...
  for (const c of d.cofres || []) {
    // `espelharSistemas` faltava aqui: o cofre restaurado voltava com o espelho
    // LIGADO (o padrão do servidor é `!== false`) e os sistemas do cliente
    // reapareciam como hosts que abrem sozinhos, mesmo tendo sido desligados.
    lines.push(`    <cofre${attrs({ apelido: c.apelido, tipo: c.tipo, nome: c.nome,
      espelharSistemas: c.espelharSistemas === false ? 'false' : 'true' })}>`);
  ...
  for (const sc of d.scripts || []) {
    // o `id` vai junto: é a identidade estável do script no round-trip (o import
    // casa por ele para não duplicar nem colapsar homônimos).
    lines.push(`    <script${attrs({ id: sc.id, name: sc.name, group: sc.group, subgroup: sc.subgroup, description: sc.description })}>${escText(sc.body || '')}</script>`);
  }
  lines.push('  </scripts>');
  ...
  // Pastas declaradas. Só as que NÃO se deduzem dos hosts/scripts precisariam
  // ir, mas a lista inteira é pequena e o import é idempotente — vai tudo, e a
  // restauração devolve a árvore exatamente como estava, vazias inclusive.
  lines.push('  <pastas>');
  for (const p of d.pastas || []) {
    if (!p || !p.group || !p.caminho) continue;
    lines.push(`    <pasta${attrs({ colecao: p.colecao || 'hosts', group: p.group, caminho: p.caminho })}/>`);
  }
  lines.push('  </pastas>');
  ...
  const s = d.settings || {};
  const bk = (s.backup && typeof s.backup === 'object' && !Array.isArray(s.backup)) ? s.backup : {};
  // Do backup automático só a retenção viaja. `pasta` é caminho desta máquina;
  // `ativo` fica de fora de propósito: a rede de segurança nasce LIGADA em toda
  // instalação — desligar é decisão por máquina, não preferência que se importa.
  const settingsAttrs = { model: s.model, termFont: s.termFont, termFontSize: s.termFontSize,
    backupManter: Number.isInteger(bk.manter) ? String(bk.manter) : undefined };
  ...
  const uiAttrs = {
    ...
    abrirLocalSozinho: ui.abrirLocalSozinho === true ? 'true' : (ui.abrirLocalSozinho === false ? 'false' : undefined),
  };
  // Filhos de <prefs>: as preferências do gerador de senhas e as pastas
  // recolhidas da lista de hosts (chave "Grupo|Rede/Core", a mesma da tela —
  // portátil porque as pastas viajam em <pastas>). Hosts recentes seguem de
  // fora: são ids desta instalação.
  const sp = (ui.senha && typeof ui.senha === 'object' && !Array.isArray(ui.senha)) ? ui.senha : {};
  const senhaAttrs = { tamanho: Number.isInteger(sp.tamanho) && sp.tamanho >= 4 && sp.tamanho <= 128 ? String(sp.tamanho) : undefined };
  for (const k of ['minusculas', 'maiusculas', 'numeros', 'simbolos', 'semAmbiguos']) senhaAttrs[k] = typeof sp[k] === 'boolean' ? String(sp[k]) : undefined;
  const temSenha = Object.values(senhaAttrs).some((v) => v !== undefined);
  const fechadas = Array.isArray(ui.pastasFechadas)
    ? ui.pastasFechadas.filter((k) => typeof k === 'string' && k && k.length < 300).slice(0, 500) : [];
  if (temSenha || fechadas.length) {
    lines.push(`  <prefs${attrs(uiAttrs)}>`);
    if (temSenha) lines.push(`    <senha${attrs(senhaAttrs)}/>`);
    for (const k of fechadas) lines.push(`    <pastaFechada${attrs({ chave: k })}/>`);
    lines.push('  </prefs>');
  } else if (Object.values(uiAttrs).some((v) => v !== undefined)) {
    lines.push(`  <prefs${attrs(uiAttrs)}/>`);
  }
