...
vncWss.on('connection', (ws, req) => {
  ...
  // Contrapressão: sem ela, um renderer lento (tela cheia, muita área suja) faz
  // a fila do WebSocket crescer dentro do processo do app até estourar memória.
  // Pausa o socket quando a fila passa de 4 MB e retoma quando drena.
  let pausado = false;
  sock.on('data', (d) => {
    if (ws.readyState !== ws.OPEN) return;
    ws.send(d, () => {
      if (pausado && !fechado && ws.bufferedAmount < 1024 * 1024) { pausado = false; sock.resume(); }
    });
    if (!pausado && ws.bufferedAmount > 4 * 1024 * 1024) { pausado = true; sock.pause(); }
  });
