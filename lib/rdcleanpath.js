...
const ERRO_GERAL = 1;     // GENERAL_ERROR_CODE no crate
const ERRO_NEGOCIACAO = 2; // NEGOTIATION_ERROR_CODE
...
// O cliente distingue Request de Response pela presença dos campos, então os
// três aqui são obrigatórios — mandar um Response sem serverAddr faz o IronRDP
// classificar o PDU como erro.
function respostaConectado({ enderecoServidor, x224, cadeiaDer }) {
  return sequencia(
    ...
    contexto(9, utf8(enderecoServidor)),
  );
}

// Erro de negociação (código 2) devolve junto o Connection Confirm original:
// é o que permite ao cliente dizer "o servidor exige CredSSP" em vez de
// "erro genérico no proxy".
function respostaErroNegociacao(x224) {
  return sequencia(
    contexto(0, inteiro(VERSAO)),
    contexto(1, sequencia(contexto(0, inteiro(ERRO_NEGOCIACAO)))),
    contexto(6, octetos(x224)),
  );
}
...
module.exports = {
  VERSAO, ERRO_GERAL, ERRO_NEGOCIACAO,
  decodificar, respostaConectado, respostaErro, respostaErroNegociacao,
};
