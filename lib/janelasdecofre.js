...
function entrada(apelido) {
  ...
  if (!e) {
    e = { clientes: new Map(), buscadoEm: 0, tentadoEm: 0, erro: null, buscando: null };
    cache.set(apelido, e);
  }
  return e;
}
...
function renovarSeVencido() {
  ...
  for (const cofre of credenciais.listaDeCofres()) {
    ...
    const podeTentar = agora - e.tentadoEm >= ESPERA_APOS_FALHA_MS;
    // Relógio remexido para trás deixa as duas contas negativas; aí renova, que
    // é o lado seguro (uma requisição a mais, e não uma janela congelada).
