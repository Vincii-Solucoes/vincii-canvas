...
// `os.userInfo()` LANÇA quando o usuário não tem entrada no passwd (contêiner,
// perfil de rede quebrado). Dentro do handler de 'connection' de um
// EventEmitter, nada captura a exceção e o processo morre — o server.js já
// protege as três outras chamadas iguais, esta ficou de fora.
function usuarioLocal() {
  try { return os.userInfo(); } catch { return { username: 'usuario', shell: null, homedir: os.homedir() }; }
}
...
wss.on('connection', (ws, req) => {
    ...
    // Mesma razão do terminal remoto: exceção solta aqui derruba o processo.
    let sessao;
    try {
      sessao = termsessions.criar({ rotulo: 'Meu computador', hostId: null });
    } catch (e) {
      enviarDireto({ t: 'e', d: `\r\n${e.message}\r\n` });
      enviarDireto({ t: 'x' });
      try { ws.close(); } catch {}
    ...
    sessao.atacar(ws);
    send({ t: 'e', d: `Terminal local — ${usuarioLocal().username}@${os.hostname()} (${shell})\r\n` });
    ...
    // EPIPE nos canos do shell chega de forma ASSÍNCRONA: o try/catch dos
    // writes abaixo não pega, e um stream sem ouvinte de 'error' derruba o
    // PROCESSO — que aqui é o main do Electron, ou seja, todas as abas, as
    // sessões SSH, o RDP e o agente em execução caem juntos. Acontece na
    // corrida entre a saída do shell e uma escrita em voo (colar um bloco
    // grande com `exit` no meio, ou fechar a aba enquanto se digita).
    // O 'close' logo abaixo continua encerrando a sessão do jeito certo.
    ...
    send({ t: 'ready' });
    // `toString('utf8')` por pacote parte caractere acentuado no meio quando os
    // bytes dele caem em pacotes diferentes — sai '' em saída em português. O
    // StringDecoder guarda os bytes incompletos para o pacote seguinte.
    const decOut = new StringDecoder('utf8');
    const decErr = new StringDecoder('utf8');
    child.stdout.on('data', (d) => send({ t: 'o', d: decOut.write(d) }));
    if (child.stderr) child.stderr.on('data', (d) => send({ t: 'o', d: decErr.write(d) }));
    child.on('error', (e) => {
