...
function criar({ rotulo, hostId }) {
  // Despeja as órfãs mais antigas antes de crescer.
  if (sessoes.size >= MAX_SESSOES) {
    const orfas = [...sessoes.values()]
      .filter((s) => !s.ws && s.orfaDesde)
      .sort((a, b) => a.orfaDesde - b.orfaDesde);
    if (orfas.length) orfas[0].encerrar('limite de sessões atingido');
  }
