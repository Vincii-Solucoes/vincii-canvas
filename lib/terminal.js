...
async function handleConnection(ws, req) {
  ...
  const host = store.get().hosts.find((h) => h.id === hostId) || quickhosts.get(hostId)
    || await dadosDeCofre.pegarHostRenovando(hostId);
  if (!host) {
    enviarDireto({ t: 'e', d: '\r\nHost não encontrado.\r\n' });
    enviarDireto({ t: 'x' });
    try { ws.close(); } catch {}
  ...
  if (host.protocol === 'telnet') {
    ...
    // Um caractere acentuado ocupa 2 bytes e pode cair partido entre dois
    // pacotes; `toString('utf8')` por pacote transformava isso em ''. O
    // StringDecoder segura o pedaço incompleto até o resto chegar — é o que a
    // aba serial já fazia e o terminal não.
    const decTelnet = new StringDecoder('utf8');
    const tn = telnet.connect({
      host: host.host,
      port: host.port || 23,
      cols, rows,
      onData: (data) => {
        const texto = decTelnet.write(data);
        send({ t: 'o', d: texto });
  ...
  runner
    ...
    .then((c) => {
      ...
      conn.shell({ term: 'xterm-256color', cols, rows }, (err, stream) => {
        ...
        // Mesmo motivo do telnet: acentuação partida na fronteira do pacote.
        const decOut = new StringDecoder('utf8');
        const decErr = new StringDecoder('utf8');
        stream.on('data', (d) => send({ t: 'o', d: decOut.write(d) }));
        stream.stderr.on('data', (d) => send({ t: 'o', d: decErr.write(d) }));
