...
rdpWss.on('connection', (ws, req) => {
  ...
  function espiarLegado(cc, restoTcp, sobraWs) {
    ...
    console.error(`[rdp-legado] CC real ${cc.toString('hex')}`);
    console.error(`[rdp-legado] CC forjado ${forjado.toString('hex')}`);
    ...
    const olhar = (quem, b) => {
      const inicio = b.slice(0, 48).toString('hex');
      let nota = '';
      // TPKT 03 00 + X.224 Data 02 f0 80, e aí o que vier
      if (b[0] === 3 && b[4] === 2 && b[5] === 0xf0 && b[6] === 0x80) {
        if (b[7] === 0x7f && b[8] === 0x65) nota = '  <<< MCS CONNECT INITIAL — HIPÓTESE CONFIRMADA';
        else if (b[7] === 0x7f && b[8] === 0x66) nota = '  <<< MCS Connect Response';
        else nota = `  (X.224 Data, primeiro byte MCS 0x${b[7].toString(16)})`;
      } else if (b[0] === 3) nota = '  (TPKT)';
      console.error(`[rdp-legado] ${quem} ${b.length}B ${inicio}${nota}`);
    };

    tcp.on('data', (d) => { olhar('servidor→', d); if (ws.readyState === ws.OPEN) ws.send(d); });
    if (restoTcp && restoTcp.length) { olhar('servidor→', restoTcp); if (ws.readyState === ws.OPEN) ws.send(restoTcp); }
    if (sobraWs && sobraWs.length) { olhar('cliente→', sobraWs); try { tcp.write(sobraWs); } catch {} }

    // Daqui em diante o repasse do cliente é cru, direto no TCP: não há TLS
    // nesta perna. O objeto imita a interface mínima que `aoReceber` usa.
    espiando = olhar;
    seguro = { writable: true, write: (b) => tcp.write(b), destroy: () => tcp.destroy() };
  }
