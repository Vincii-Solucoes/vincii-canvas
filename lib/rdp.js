...
// Modo RDP legado (Standard Security). Ligado POR HOST, nunca por acidente: o
// servidor não é autenticado e a criptografia é a antiga do RDP. A variável de
// ambiente continua existindo para testes, mas o caminho normal é a caixa
// "Permitir RDP legado" no cadastro do host.
const LEGADO_POR_ENV = process.env.VC_RDP_LEGADO === '1';
  ...
  function conectar(x224Cliente, sobra, cifras) {
    ...
    const aoResponder = (d) => {
      ...
      if (neg.tipo === 'ok' && neg.protocolo === 0) {
        if (LEGADO_POR_ENV || host.rdpLegado === true) {
        ...
        return erroNegociacao(cc, `${alvo}:${porta} está em RDP legado, sem TLS. `
          + 'Habilite TLS no servidor, ou marque "Permitir RDP legado" no cadastro deste host.');
      }
