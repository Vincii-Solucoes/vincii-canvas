...
const autologon = require('./rdp-autologon');
const protocolos = require('../public/protocolos');

const CONNECT_TIMEOUT_MS = 15000;
const TLS_TIMEOUT_MS = 15000;
// No modo legado não há handshake TLS, então nenhum dos prazos acima cobre a
// fase de conexão. Sem isto, um servidor que aceita a conexão e depois cala
// (ou manda um TPKT que nunca completa) deixa o socket pendurado para sempre.
// Medido com um servidor hostil: a sessão ficava aberta indefinidamente.
const LEGADO_HANDSHAKE_MS = Number(process.env.VC_RDP_HS_MS || 30000);
...
// O tipo de segurança do RDP é DETECTADO, não configurado: o servidor diz na
// negociação X.224 o que ele fala, e o proxy se adapta. TLS/CredSSP quando o
// servidor oferece; Standard Security (RC4) quando ele só tem isso.
//
// O usuário não deveria precisar saber a diferença — mas precisa ser AVISADO
// quando cai no legado, porque ali o servidor não é autenticado. Por isso
// registramos o modo de cada conexão para a interface poder mostrar o aviso.
// Guarda também a ÚLTIMA FALHA: o motivo do fechamento do WebSocket não chega
// ao cliente durante a fase de RDCleanPath (medido — o IronRDP só enxerga
// "faltam bytes"), e todo erro nosso vira um PDU genérico que ele classifica
// com o mesmo código. Sem isto, um timeout era indistinguível de "precisa
// consentir com o modo antigo" — e o app entrava em laço perguntando.
const modoDaConexao = new Map(); // hostId → { modo, erro, em }
function registrarModo(hostId, modo, erro) {
  modoDaConexao.set(hostId, { modo, erro: erro || null, em: Date.now() });
  // não deixa a memória crescer sem limite numa sessão longa
  if (modoDaConexao.size > 200) {
    const maisVelho = [...modoDaConexao.entries()].sort((a, b) => a[1].em - b[1].em)[0];
    if (maisVelho) modoDaConexao.delete(maisVelho[0]);
  }
}
function modoDe(hostId) {
  const r = modoDaConexao.get(hostId);
  return r ? { modo: r.modo, erro: r.erro } : { modo: null, erro: null };
}

// Consentimento para falar RDP antigo com um host específico.
//
// O tipo do RDP é detectado, não configurado — mas cair no modo antigo tem
// consequência: o servidor não é autenticado, e é para ele que a senha vai
// dentro do Client Info. Pior, quem estiver no caminho de rede pode FORÇAR o
// rebaixamento reescrevendo 4 bytes do Connection Confirm, e colher a
// credencial de um servidor que falaria TLS.
//
// Por isso a conexão é recusada na primeira vez e só segue depois que o
// usuário confirma, uma vez por host. A senha não sai da máquina antes disso:
// o Client Info só é montado depois da negociação.
const consentidos = new Set();
function consentiu(hostId) { return consentidos.has(hostId); }
function consentir(hostId) { consentidos.add(hostId); }
...
function acharHost(id) {
  // O espelhado entra por último na cadeia, como em toda resolução de host —
  // sem ele aqui, o RDP espelhado abria a aba, buscava a credencial e morria
  // na ponte com "Host não encontrado".
...
function tpktCompleto(buf) {
  ...
  // Um TPKT válido tem no mínimo o cabeçalho de 4 bytes mais o X.224. Sem esta
  // guarda, um servidor que declara comprimento 0 faz `buf.length >= 0` dar
  // verdadeiro com total 0 — o chamador entende "incompleto", não consome nada,
  // e o buffer cresce até o processo morrer. Como o servidor roda dentro do
  // processo principal do Electron, isso derrubaria o app inteiro e as sessões
  // SSH abertas junto. O módulo irmão rdp-quadros.js já fazia esta checagem.
  if (total < 7) return -1;
...
rdpWss.on('connection', (ws, req) => {
  ...
  // No modo legado, encaminha o que vem do WebSocket para o tradutor.
  let encaminharCliente = null;

  // ---------- autologon no caminho TLS ----------
  // O modo legado conserta o Client Info dentro do tradutor. Aqui não há
  // tradutor, então os bytes do cliente passam por uma espia que enquadra o
  // mínimo necessário até achar esse PDU. A lógica mora em rdp-autologon.js,
  // onde dá para testá-la sem subir proxy nenhum.
  const escreverNoTls = (b) => {
    if (seguro && seguro.writable) { try { seguro.write(b); } catch {} }
  };
  const paraTlsComAutologon = autologon.criarEspia({
    escrever: escreverNoTls,
    log: (r) => {
      if (!process.env.VC_RDP_DEBUG) return;
      console.error(`[rdp] Client Info (TLS): ${r.marcado
        ? `autologon marcado (flags 0x${r.antes.toString(16)} → 0x${r.depois.toString(16)})`
        : `autologon não marcado — ${r.motivo}`}`);
    },
  });
  ...
  const erroProtocolo = (opcoes, msg) => {
    if (fechado) return;
    if (host) registrarModo(host.id, 'falhou', msg);
  ...
  // genérico do proxy.
  const erroNegociacao = (cc, msg, modo) => {
    if (fechado) return;
    if (host) registrarModo(host.id, modo || 'falhou', msg);
  ...
  const porta = host.port || protocolos.portaPadrao('rdp');
  // Zera o registro no início de CADA tentativa. Sem isto, o resultado de uma
  // conexão anterior (por exemplo "precisa consentir") seria lido como se fosse
  // desta, e o app perguntaria de novo por um motivo que não existe mais.
  registrarModo(host.id, 'conectando');
  ...
  const aoReceber = (data) => {
    ...
    if (!emHandshake) {
      ...
      if (encaminharCliente) { encaminharCliente(b); return; }
      paraTlsComAutologon(b);
      return;
    }
  ...
  function conectar(x224Cliente, sobra, cifras) {
    ...
    const aoResponder = (d) => {
      ...
      // Defesa em profundidade: o Connection Confirm tem ~20 bytes. Se chegou a
      // 64 KB, o outro lado não está falando RDP com a gente.
      if (respostaX224.length > 64 * 1024) {
        return erroProtocolo({}, `${alvo}:${porta} mandou resposta X.224 grande demais.`);
      }
      ...
      // O servidor só fala Standard Security: usa o tradutor. Não é escolha do
      // usuário — é o que a máquina do outro lado oferece. Mas a primeira vez
      // exige confirmação, porque aí a senha vai para um servidor cuja
      // identidade ninguém verificou.
      if (neg.tipo === 'ok' && neg.protocolo === 0) {
        if (!consentiu(host.id) && !host.rdpLegadoOk) {
          return erroNegociacao(cc,
            `${alvo}:${porta} só oferece RDP antigo, sem verificar identidade.`,
            'legado-pendente');
        }
        registrarModo(host.id, 'legado');
        return modoLegado(cc, respostaX224.slice(total), sobra);
      }
      registrarModo(host.id, 'tls');
  ...
  // Connect Initial.
  function modoLegado(cc, restoTcp, sobraWs) {
    ...
    // Prazo para a sessão sair do handshake. Cai fora assim que a criptografia
    // é estabelecida — depois disso uma sessão quieta é normal e não deve morrer.
    let relogioLegado = setTimeout(() => {
      const msg = `${alvo}:${porta} aceitou a conexão mas não completou o handshake RDP.`;
      registrarModo(host.id, 'falhou', msg);
      abortar(msg);
    }, LEGADO_HANDSHAKE_MS);
    const cancelarRelogio = () => { if (relogioLegado) { clearTimeout(relogioLegado); relogioLegado = null; } };
    ws.on('close', cancelarRelogio);
    ...
    const bombear = (acc, chegou, traduzir, entregar, quem) => {
      ...
      for (;;) {
        ...
        acc.b = acc.b.slice(n);
        // A entrega fica DENTRO do try: um ws.send ou tcp.write que lance aqui
        // subiria pela pilha do handler de 'data' e derrubaria o processo.
        try {
          const saida = traduzir(pdu);
          for (const p of (Array.isArray(saida) ? saida : [saida])) entregar(p);
        } catch (e) { return abortar(`RDP legado: ${e.message}`); }
        if (relogioLegado && tradutor.estado.fase === 'cifrado') cancelarRelogio();
      }
    };
    ...
    encaminharCliente = (b) => bombear(bufCliente, b, (p) => tradutor.doCliente(p), paraServidor, 'cliente');
    if (sobraWs && sobraWs.length) encaminharCliente(sobraWs);
    // O tradutor já escreveu no TCP; `seguro` existe só para o encerramento
    // achar o socket certo.
    seguro = { writable: false, write: () => {}, destroy: () => tcp.destroy() };
  }
  ...
  function subirTls(cc, restoTcp, sobraWs, x224Cliente, cifras) {
    ...
    // Mesma contrapressão da ponte VNC (lib/desktop.js): renderer lento não
    // pode fazer a fila do WebSocket crescer sem limite no processo do app.
    let pausadoRdp = false;
    seguro.on('data', (d) => {
      if (ws.readyState !== ws.OPEN) return;
      ws.send(d, () => {
        if (pausadoRdp && !fechado && ws.bufferedAmount < 1024 * 1024) {
          pausadoRdp = false;
          try { if (seguro) seguro.resume(); } catch {}
        }
      });
      if (!pausadoRdp && ws.bufferedAmount > 4 * 1024 * 1024) {
        pausadoRdp = true;
        try { seguro.pause(); } catch {}
      }
    });
    seguro.on('error', (err) => {
      clearTimeout(relogio);
...
module.exports = { rdpWss, modoDe, consentiu, consentir };
