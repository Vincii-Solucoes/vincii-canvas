...
const modoDaConexao = new Map(); // hostId → 'legado' | 'tls'
function registrarModo(hostId, modo) {
  modoDaConexao.set(hostId, { modo, em: Date.now() });
...
rdpWss.on('connection', (ws, req) => {
    ...
    const aoResponder = (d) => {
      ...
      if (neg.tipo === 'ok' && neg.protocolo === 0) {
        if (!consentiu(host.id) && !host.rdpLegadoOk) {
          registrarModo(host.id, 'legado-pendente');
          return erroNegociacao(cc, `${alvo}:${porta} só oferece RDP antigo, sem verificar identidade.`);
        }
  ...
  function modoLegado(cc, restoTcp, sobraWs) {
    ...
    let relogioLegado = setTimeout(() => {
      abortar(`${alvo}:${porta} aceitou a conexão mas não completou o handshake RDP.`);
    }, LEGADO_HANDSHAKE_MS);
