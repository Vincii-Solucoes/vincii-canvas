...
// Cópia LATERAL do caminho da pasta, fora do data.json: no arranque corrompido
// ou apagado o settings.backup morreu junto com os dados — e é exatamente aí
// que a tela precisa listar as cópias para a pessoa restaurar. Sem isto, quem
// tinha pasta personalizada via a lista VAZIA (a padrão) no dia do desastre e
// precisava lembrar o caminho de cabeça. Só é lida nesse arranque; no normal,
// o settings manda e o lateral é apenas espelho.
function arquivoLateral() { return path.join(store.diretorio(), '.canvas-backup-pasta'); }
function lerLateral() {
  try { const p = fs.readFileSync(arquivoLateral(), 'utf8').trim(); return path.isAbsolute(p) ? p : ''; }
  catch { return ''; }
}
function gravarLateral(pasta) {
  try {
    if (pasta) fs.writeFileSync(arquivoLateral(), String(pasta), { mode: 0o600 });
    else fs.rmSync(arquivoLateral(), { force: true }); // voltou à padrão: nada a lembrar
...
function cfg() {
  ...
  let pasta = (typeof b.pasta === 'string' && b.pasta.trim()) ? b.pasta.trim() : '';
  if (!pasta && protegido()) pasta = lerLateral(); // o settings morreu com o data.json
  if (!pasta) pasta = pastaPadrao();
...
function garantirPasta(pasta) {
  // 0o700: os snapshots dentro já nascem 0600; a pasta não pode ser a parte
  // visível. Pasta já existente NÃO é alterada — é do usuário. Windows ignora.
  fs.mkdirSync(pasta, { recursive: true, mode: 0o700 });
}

// Sobra de uma queda entre writeFileSync(tmp) e renameSync: um
// canvas-data-*.json.tmp que listar() não vê e podar() nunca apaga. Só na hora
// de GRAVAR (nunca em leitura, nunca em modo protegido), só o que é nosso, e só
// com mais de 1 min — um .tmp recente pode ser de outra instância na mesma pasta.
function limparTemporarios(pasta) {
  // Relógio REAL de propósito: o mtime do arquivo é real, e o `agora` que o
  // motor recebe pode ser um instante lógico (os testes controlam o tempo).
  const agora = Date.now();
  let nomes;
  try { nomes = fs.readdirSync(pasta); } catch { return; }
  for (const nome of nomes) {
    if (!nome.startsWith(PREFIXO) || !nome.endsWith(SUFIXO + '.tmp')) continue;
    const alvo = path.join(pasta, nome);
    try {
      const st = fs.lstatSync(alvo);
      if (st.isFile() && !st.isSymbolicLink() && agora - st.mtimeMs > 60_000) fs.unlinkSync(alvo);
    } catch { /* sumiu ou não é nosso: ignora */ }
  }
}
...
function aoIniciar(agora = Date.now()) {
  // No arranque BOM o settings manda e o lateral é espelhado — assim uma
  // instalação que já tinha pasta personalizada fica protegida sem precisar
  // re-salvar a config, e restaurar um backup antigo não deixa ponteiro velho.
  if (!protegido()) {
    const b = (store.get().settings || {}).backup;
    const p = b && typeof b.pasta === 'string' ? b.pasta.trim() : '';
    if (lerLateral() !== p) gravarLateral(p);
  }
...
function aplicar(body) {
  ...
  store.save();
  // Só quando a pasta veio no pedido: no arranque protegido, um PUT só de
  // ativo/manter resolve nova.pasta como '' e apagaria o lateral à toa.
  if (body.pasta !== undefined) gravarLateral(nova.pasta);
...
function restaurar(nome) {
  ...
  // Conferir o alvo ANTES de guardar a cópia do estado atual: uma recusa não
  // pode deixar arquivo novo na pasta (a mensagem diz "Nada foi alterado") nem
  // consumir vaga da retenção a cada tentativa.
  try {
    store.validarBackup(fs.readFileSync(alvo, 'utf8'));
  } catch (e) {
    return { ok: false, erro: `Não foi possível restaurar: ${e.message}` };
  }
  // Cópia do estado ATUAL antes de sobrescrever. Sem ela, escolher a cópia
  // errada apaga tudo o que foi feito desde o último snapshot — e o automático
  // roda a cada 12 h — sem nenhum caminho de volta.
  //
  // De propósito NÃO chama copiar(): o podar() de lá respeita a retenção e, com
  // "manter 1", poderia apagar justamente o `alvo` antes de ele ser lido. Esta
  // cópia extra é podada sozinha no próximo arranque. E não roda com o arranque
  // protegido (data.json corrompido/ausente): ali o estado atual é o vazio, e
  // guardá-lo como se fosse bom é o oposto do que se quer.
  if (!protegido()) {
    const destino = path.join(pasta, nomeDoInstante(Date.now()));
    const tmpPre = destino + '.tmp';
    try {
      const atual = fs.readFileSync(store.arquivo(), 'utf8');
      JSON.parse(atual); // não guardar lixo com cara de estado bom
      garantirPasta(pasta);
      fs.writeFileSync(tmpPre, atual, { mode: 0o600 });
      fs.renameSync(tmpPre, destino);
      console.error(`[backup] estado anterior guardado em ${path.basename(destino)} antes de restaurar`);
    } catch {
      // Melhor esforço: nunca abortar a restauração por causa da cópia — mas
      // não deixar o .tmp para trás.
      try { fs.unlinkSync(tmpPre); } catch { /* nem chegou a existir */ }
    }
  }
  try {
