...
// O arquivo EXISTE mas não deu para lê-lo agora (Keychain recusou, app
// reassinado, rodando fora do Electron). `ler()` devolve {} nesses casos — e
// devolver {} é certo, as chaves continuam no disco. O que NÃO pode acontecer é
// o `gravar()` seguinte regravar o arquivo a partir desse {} e destruir as
// chaves dos outros cofres. Esta marca é o que impede.
let ilegivel = false;

function ler() {
  if (!fs.existsSync(ARQUIVO)) { ilegivel = false; return {}; }
  let cru;
  try { cru = fs.readFileSync(ARQUIVO); } catch { ilegivel = true; return {}; }
  try {
    ...
    if (env && env.formato === 'protegido') {
      ...
      const aberto = JSON.parse(api.decifrar(Buffer.from(env.dados, 'base64')));
      ilegivel = false;
      return aberto;
    }
    ilegivel = false;
...
function gravar(mapa) {
  // Recusa gravar por cima de um arquivo que não conseguimos LER: o `mapa` foi
  // montado a partir de um {} e regravá-lo apagaria as chaves de todos os
  // outros cofres — perda silenciosa e definitiva. Melhor um erro na tela.
  if (ilegivel && fs.existsSync(ARQUIVO)) {
    throw new Error(`As chaves guardadas em ${ARQUIVO} não puderam ser lidas agora, `
      + 'então nada foi gravado — regravar apagaria as que já estão lá. Abra pelo app '
      + 'desktop e libere o acesso ao chaveiro do sistema; se quiser mesmo recomeçar '
      + 'do zero, apague esse arquivo.');
  }
