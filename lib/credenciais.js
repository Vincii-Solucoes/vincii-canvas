...
async function resolver(host) {
  ...
  return {
    // A forma segue o CAMPO, não o nome do tipo: o contrato tem só duas formas
    // (valor único em `senha`; PEM em `chavePrivada`) e promete que tipo novo
    // chega numa delas. Decidir pelo nome fazia "certificado" virar password
    // vazio, e faria o mesmo com qualquer tipo futuro.
    type: s.chavePrivada ? 'key' : 'password',
    password: s.chavePrivada ? '' : (s.senha || ''),
    privateKey: s.chavePrivada || null,
    ...
    usuario: s.usuario || '',
    // O domínio do Active Directory (14/ago). Mesma regra do usuário: o campo
    // digitado no host manda, o do cofre cobre quando vazio.
    dominio: s.dominio || '',
...
module.exports = {
  // `comRecuo` é exportada para o cache de espelho (lib/dadosdecofre.js) poder
  // usar a MESMA política de retentativa das conexões — sem ela, um 502 que o
  // adaptador marca como transitório sumia sem uma tentativa a mais.
  comRecuo,
  resolver,
  ping,
  listarSegredos,
