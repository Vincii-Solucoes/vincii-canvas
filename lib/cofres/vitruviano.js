...
// Os dois estados de "cliente sem janela declarada" (16/08). Valor estranho é
// descartado — a tela decide texto por estado, e um estado desconhecido viraria
// frase sem sentido.
const SEM_JANELA = ['sem_turnos', 'encaminhamento_desativado'];
...
const CODIGOS = {
  ...
  // Analista DESLIGADO (entregue em 16/08): antes, a listagem sem filtro
  // devolvia 200 com lista vazia e a tela sugeria "peça acesso ao cliente"
  // quando o problema era a conta inteira. Definitivo — insistir não recontrata.
  funcionario_inativo: { transitorio: false },
...
module.exports = {
  ...
  async ping(cfg) {
    ...
    return {
      ...
      cofres: (Array.isArray(r.cofres) ? r.cofres : []).map((c) => ({
        ...
        // Quando a janela NÃO vem, este campo (entregue em 16/08) diz QUAL dos
        // dois estados é — e a trava por horário vale nos dois:
        //   sem_turnos: cliente sem turno cadastrado; a credencial nunca abre;
        //   encaminhamento_desativado: turnos existem, o admin desligou o
        //     encaminhamento; abre por clique, dentro do horário.
        // Antes dele, os dois casos eram uma ausência muda — e o pior ficava
        // invisível: o cliente TEM trava, o Canvas só não sabia qual.
        semJanela: SEM_JANELA.includes(c && c.semJanela) ? c.semJanela : '',
      })).filter((c) => c.id),
    };
  },
  ...
  async ler(cfg, id) {
    ...
    const comum = {
      ...
      // E quando NÃO vem, `semJanela` (16/08) diz o estado — igual ao ping.
      semJanela: SEM_JANELA.includes(r.semJanela) ? r.semJanela : '',
    };
