...
// Sentinela gravada como pino quando a cadeia do certificado é CONFIÁVEL. Sem
// ela, um cofre com certificado de CA pública nunca ganhava pino — e no dia em
// que um autoassinado aparecesse no lugar dele, o TOFU abaixo fixaria o
// impostor e mandaria o token. Com a sentinela, cair de "confiável" para "não
// confiável" é uma TROCA de certificado, e troca é recusada. Nunca colide com
// uma impressão real, que começa com 'sha256/'.
const CADEIA_CONFIAVEL = 'cadeia-confiavel';
  ...
  return new Promise((resolve, reject) => {
    ...
    const socket = tls.connect({
      ...
      if (socket.authorized) {
        // Cadeia confiável (CA pública): a validação normal basta, e o que se
        // guarda é a SENTINELA — o registro de que este cofre é do tipo que
        // apresenta certificado válido. É isso que permite recusar o dia em que
        // ele passar a apresentar um autoassinado.
        if (!cfg.certificadoFixado && typeof aoFixar === 'function') aoFixar(CADEIA_CONFIAVEL);
      } else if (cfg.certificadoFixado) {
        if (atual !== cfg.certificadoFixado) {
          socket.destroy();
          return reject(new ErroDeCofre('certificado_mudou',
            cfg.certificadoFixado === CADEIA_CONFIAVEL
              ? 'O cofre sempre respondeu com certificado de CA confiável e agora apresenta '
                + 'um que NÃO é (autoassinado, vencido, ou de outra CA). Se você não trocou o '
                + 'servidor nem deixou o certificado vencer, alguém está no meio do caminho — '
                + 'não use "esquecer certificado" nesta rede.'
