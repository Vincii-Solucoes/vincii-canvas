...
const DEFAULTS = { globals: {}, hosts: [], playbooks: [], scripts: [], capturas: [], pastas: [], profiles: [], favorites: [], settings: {} };
...
function load() {
  ...
  try {
    ...
    const d = migrar({ ...DEFAULTS, ...parsed });
    // A lista de pastas pode vir de arquivo editado à mão: só entra o que é
    // pasta de verdade, sem duplicata e dentro do teto. E as pastas em que os
    // hosts já estão passam a existir por conta própria — dados de antes desta
    // lista tinham pastas só derivadas, que sumiriam ao esvaziar.
    const pastas = require('./pastas');
    d.pastas = pastas.sanear(d.pastas);
    pastas.completarDosItens(d.pastas, d);
    return d;
  } catch (err) {
    const copia = FILE + '.corrompido-' + Date.now();
    // A cópia herda o modo do original; se ele estava largado em 0644, a cópia
    // (com credenciais) também ficaria. Restringe na mesma tentativa.
    try { fs.copyFileSync(FILE, copia); fs.chmodSync(copia, 0o600); } catch { /* sem espaço/permissão: seguimos */ }
    // Cada arranque com o arquivo ilegível deixa mais uma cópia — e cada cópia
    // tem as credenciais dentro. Guarda as 5 mais novas e diz quantas existem,
    // em vez de encher a pasta em silêncio.
    try {
      const dir = path.dirname(FILE);
      const nossas = fs.readdirSync(dir)
        .filter((f) => f.startsWith(path.basename(FILE) + '.corrompido-'))
        .sort();
      if (nossas.length > 5) {
        for (const velha of nossas.slice(0, nossas.length - 5)) {
          try { fs.unlinkSync(path.join(dir, velha)); } catch {}
        }
      }
      console.warn(`[store] ${Math.min(nossas.length, 5)} cópia(s) de data.json corrompido em ${dir} `
        + '(contêm credenciais — apague quando não precisar mais).');
    } catch {}
    diagnosticoDeArranque = {
      tipo: 'corrompido',
      copia,
...
// Validar só "é JSON" deixava passar um array, uma string ou o arquivo de
// outro programa: o app reabria VAZIO, sem diagnóstico, com o backup bom já
// sobrescrito. O que se exige é o formato deste app. Exportada para o motor de
// backup conferir o alvo ANTES de guardar a cópia do estado atual.
function validarBackup(bruto) {
  const lido = JSON.parse(bruto);
  if (!lido || typeof lido !== 'object' || Array.isArray(lido)) {
    throw new Error('Este arquivo não é um backup do Vincii Canvas (o conteúdo não é um objeto).');
  }
  if (!Array.isArray(lido.hosts)) {
    throw new Error('Este arquivo não é um backup do Vincii Canvas: falta a lista de hosts. '
      + 'Nada foi alterado.');
  }
  return lido;
}
...
function restaurarDeArquivo(caminho) {
  ...
  console.warn('[store] restauração aplicada no disco — as gravações desta sessão estão '
    + 'TRAVADAS até o app reiniciar (senão o estado antigo, ainda em memória, voltaria por cima).');
}
...
module.exports = { get, save, diretorio, arquivo, diagnostico, restaurarDeArquivo, validarBackup };
