...
function connect({ host, port, term = 'xterm-256color', cols = 80, rows = 24, onData, onError, onClose }) {
  ...
  return {
    ...
    resize(c, r) {
      // Guarda o tamanho atual: um `DO NAWS` que chegue DEPOIS deste resize
      // (servidores lentos negociam tarde) responderia com o tamanho INICIAL
      // da sessão, e a tela remota nasceria com a largura errada.
      cols = c; rows = r;
      sendWindowSize(c, r);
    },
