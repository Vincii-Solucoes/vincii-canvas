...
const { estadoDeErro } = require('./redetools');
...
// 'inacessivel' (sem rota — na LAN costuma ser a permissão de Rede Local
// do macOS negada ao app; ver estadoDeErro em redetools).
function sondar(host, porta, timeoutMs = TIMEOUT_MS) {
  return new Promise((resolve) => {
    ...
    s.once('error', (e) => fim(estadoDeErro(e.code) || 'timeout'));
...
async function rodada() {
  ...
  await Promise.all([...alvos.values()].map(async (a) => {
    ...
    if (r.estado === 'aberta') {
      ...
      a.perdidos += 1; a.ultima = null; a.status = r.estado; // 'fechada' | 'timeout' | 'inacessivel'
    }
