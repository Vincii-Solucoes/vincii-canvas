...
function segredosConhecidos() {
  ...
  // Hosts AVULSOS (conexão rápida) vivem só na memória do quickhosts, nunca no
  // data.json — então a varredura acima não os pega. Sem isto, a senha de um
  // host avulso podia ir em texto claro para a API da IA (o retrato da tela do
  // Telnet, um `echo $PASS`, um erro de conexão). Require preguiçoso, como vivos().
  try {
    for (const h of require('./quickhosts').listar()) {
      if (h.auth && h.auth.password) out.push(h.auth.password);
      if (h.auth && h.auth.passphrase) out.push(h.auth.passphrase);
    }
  } catch { /* quickhosts indisponível: segue */ }
  out.push(...vivos());
