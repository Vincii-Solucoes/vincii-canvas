...
function buildSystem(host) {
  if (host.local) {
    ...
    return (
      ...
      `Máquina: ${host.username}@${host.host} — sistema operacional: ${osName}. Use comandos compatíveis com esse sistema. ` +
...
async function runAgent(run, { host, goal, options, saveData }) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    for (const block of final.content) {
      ...
      // Na MÁQUINA LOCAL todo comando pede aprovação a partir do momento em que
      // o agente já leu alguma saída: essa saída é dado não confiável (log de um
      // servidor, arquivo baixado) e pode conter instruções tentando fazer o
      // modelo rodar algo aqui. Lista de comandos "perigosos" é só um rótulo —
      // nenhuma regex cobre todas as grafias —, então a barreira real é o aviso
      // ao analista antes de executar.
      const danger = dangerousReason(cmd);
      const exigeAprovacao = danger
        ? options.confirmDangerous
        : (isLocal && run.leuSaidaExterna);
      if (exigeAprovacao) {
        emit(run, {
          type: 'need-approval',
          id: block.id,
          command: cmd,
          reason: danger || 'comando na SUA máquina depois de ler saída de outro comando (pode ter sido influenciado por ela)',
        });
      ...
      try {
        result = isLocal
          ...
          : await runner.execCommand(conn, cmd, execOpts);
      } catch (e) {
...
module.exports = { start, stop, approve, getRun, subscribe, unsubscribe, dangerousReason, execLocal };
