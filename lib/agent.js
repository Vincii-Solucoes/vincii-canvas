...
function redigir(texto) {
  ...
  try {
    ...
    // Credencial vinda de COFRE não está no data.json — é buscada na hora e vive
    // só em memória. Sem esta linha, a lista acima ficaria vazia para todo host
    // que usa cofre, e uma senha que aparecesse na saída de um comando iria em
    // TEXTO CLARO para a API da Anthropic. É o buraco que a integração de cofre
    // abre nesta função, e é por isso que `resolver()` mantém o valor
    // registrado enquanto a sessão existe.
    segredos.push(...credenciais.segredosVivos());
  } catch {}
