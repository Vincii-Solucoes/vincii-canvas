...
// Todos os avulsos de um protocolo. Usado para casar a sessão do webview com o
// host que a abriu, sem depender do endereço.
function listar(protocolo) {
  cleanup();
  const out = [];
  for (const h of hosts.values()) if (!protocolo || h.protocol === protocolo) out.push(h);
  return out;
}

module.exports = { add, get, acharPorEndereco, listar };
