...
const MAX = { name: 120, group: 80, subgroup: 200, description: 400, body: 100000 };
...
function normalizar(entrada) {
  ...
  // Pasta só existe DENTRO de um grupo — mesma invariante dos hosts
  // (agrupar.js). Sem grupo, a pasta é descartada em vez de virar uma seção
  // órfã dentro de "Sem grupo". E é um CAMINHO ("Rede/Core"), normalizado pela
  // mesma função que desenha a árvore.
  const subgroup = group ? agrupar.limitarCaminho(e.subgroup, MAX.subgroup) : '';
  ...
  for (const raw of Array.isArray(entradas) ? entradas : []) {
    ...
    if (!ex) {
      ex = destino.find((x) => !usados.has(x.id)
        && x.name === v.name && (x.group || '') === v.group && agrupar.normalizarCaminho(x.subgroup) === v.subgroup);
    }
