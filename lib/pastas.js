...
// `existentes` (opcional): o Set de chaves da lista, para quem chama em laço
// (completarDosItens, import) não reconstruí-lo a cada item — era O(hosts ×
// pastas) no arranque: 300 ms com 800 hosts e 1500 pastas.
function declarar(lista, colecao, group, caminho, existentes) {
  const base = normalizar({ colecao, group, caminho });
  if (!base) return 0;
  if (!existentes) existentes = new Set(lista.map(chave));
...
function mover(lista, colecao, group, de, para) {
  ...
  // "Rede" para "Infra/Rede" deixaria "Infra" sem declaração própria. Só
  // quando algo de fato se moveu: mover uma pasta que não existe não pode
  // inventar o destino (nem um grupo novo).
  if (destino && n) declarar(lista, origem.colecao, origem.group, destino);
  return n;
}
...
function completarDosItens(lista, d) {
  let n = 0;
  const existentes = new Set(lista.map(chave)); // um Set para a carga inteira
  for (const h of (d && Array.isArray(d.hosts)) ? d.hosts : []) {
    if (h && h.group && h.subgroup) n += declarar(lista, 'hosts', h.group, h.subgroup, existentes);
  }
  for (const s of (d && Array.isArray(d.scripts)) ? d.scripts : []) {
    if (s && s.group && s.subgroup) n += declarar(lista, 'scripts', s.group, s.subgroup, existentes);
  }
  return n;
}

module.exports = { chave, normalizar, declarar, remover, mover, das, sanear, completarDosItens, MAX_DECLARADAS, MAX_CAMINHO, MAX_GRUPO };
