...
// shell. Como o ping é chamado com args (sem shell), já não há injeção de
// SHELL; o `!/^-/` barra injeção de ARGUMENTO: sem ele, "-c5"/"-i…" passava e o
// ping o lia como FLAG, não como alvo. Mesma guarda dos irmãos (tcpping, mtr,
// redetools). Também barra lixo antes de gastar um processo.
function ipValido(ip) {
  return typeof ip === 'string' && ip.length > 0 && ip.length <= 255
    && /^[a-zA-Z0-9.:_-]+$/.test(ip) && !/^-/.test(ip);
}
...
// Ver o comentário igual em lib/mtr.js: rodada assíncrona mais lenta que o
// intervalo abria uma rodada por cima da outra.
let emCurso = false;
function iniciarLoop() {
  ...
  timer = setInterval(() => {
    if (emCurso) return;
    emCurso = true;
    Promise.resolve(rodada()).catch(() => {}).finally(() => { emCurso = false; });
  }, INTERVALO_MS);
