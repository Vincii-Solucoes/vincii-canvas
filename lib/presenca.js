...
// Alguém ALÉM de `exceto` está com este host na tela?
//
// `exceto` é sempre a janela que pergunta: ela já sabe das próprias abas, e sem
// esse recorte toda janela concluiria "já tem alguém com esse host" olhando para
// o próprio reflexo — e nunca abriria nada.
function mostrandoPorOutra(hostId, exceto, agora = agoraMs()) {
  const alvo = String(hostId || '');
  if (!alvo) return false;
  const fora = String(exceto || '');
  return listar(agora).some((p) => p.hostId === alvo && p.janela !== fora);
}

function limpar() { janelas.clear(); }

module.exports = { anunciar, sair, listar, mostrandoPorOutra, limpar, TTL_MS, MAX_JANELAS };
