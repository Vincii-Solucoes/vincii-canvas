'use strict';

const MAX_EVENTS = 4000; // eventos guardados por execução (replay do SSE)

const fs = require('fs');
const os = require('os');
const crypto = require('crypto');
const net = require('net');
const { Client } = require('ssh2');
const credenciais = require('./credenciais');
const dadosDeCofre = require('./dadosdecofre');

const MAX_RUNS = 30;
const MAX_OUTPUT_BYTES = 512 * 1024; // por stream, por comando
// Duas fases, dois relógios — porque "timeout" tinha DOIS sentidos que a tela
// não distinguia: "não cheguei nem a abrir a conexão até o host" (rede: IP
// errado, Wi-Fi dormindo, firewall) e "abri a conexão mas o SSH não respondeu"
// (servidor: sshd lento/sobrecarregado, ou nem é SSH nessa porta). O `ssh`
// nativo só limita a fase TCP; nós limitávamos o handshake INTEIRO em 12 s, e
// no timeout mandávamos o usuário adivinhar de qual lado estava o problema.
const TCP_CONNECT_TIMEOUT_MS = 10000;   // abrir o socket (Wi-Fi de Deck acorda devagar)
const SSH_HANDSHAKE_TIMEOUT_MS = 15000; // depois do TCP: KEX + autenticação
const PARALLEL_HOSTS = 5;

const runs = new Map();

function writeEvent(res, event) {
  res.write(`data: ${JSON.stringify(event)}\n\n`);
}

function emit(run, event) {
  event.ts = Date.now();
  run.events.push(event);
  // teto: sem isso a coleta de log em lote retém toda a saída em memória e o
  // replay do SSE (para quem abre a tela depois) trava o event loop
  if (run.events.length > MAX_EVENTS) run.events.splice(0, run.events.length - MAX_EVENTS);
  for (const res of run.subscribers) writeEvent(res, event);
}

function getRun(id) {
  return runs.get(id);
}

function subscribe(run, res) {
  for (const event of run.events) writeEvent(res, event);
  if (run.status !== 'executando') {
    res.end();
    return;
  }
  run.subscribers.add(res);
}

function unsubscribe(run, res) {
  run.subscribers.delete(res);
}

function expandHome(p) {
  if (!p) return p;
  return p === '~' ? os.homedir() : p.replace(/^~(?=\/)/, os.homedir());
}

function humanizeError(err, host, ctx = {}) {
  const msg = String((err && err.message) || err);
  const alvo = `${host.host}:${host.port || 22}`;
  if (/All configured authentication methods failed/i.test(msg)) {
    return 'Falha de autenticação — verifique usuário, senha ou chave.';
  }
  if (/ECONNREFUSED/.test(msg)) return `Conexão recusada em ${alvo} — o SSH está ativo nessa porta?`;
  if (/ENOTFOUND|EAI_AGAIN/.test(msg)) return `Host não encontrado: ${host.host}`;
  if (/Timed out|ETIMEDOUT/i.test(msg)) {
    // Aqui o TCP JÁ estava aberto (a fase de rede passou): o silêncio é do
    // servidor SSH, não da rede. Antes esta mensagem era a mesma dos dois casos.
    return ctx.tcpOk
      ? `Conectei em ${alvo}, mas o servidor SSH não respondeu ao handshake a tempo — `
        + 'o sshd pode estar lento ou sobrecarregado, ou pode não ser SSH nessa porta.'
      : 'Tempo esgotado ao conectar.';
  }
  if (/Cannot parse privateKey/i.test(msg)) return 'Não foi possível ler a chave privada (formato ou passphrase incorretos).';
  if (/ENOENT/.test(msg)) return 'Arquivo de chave privada não encontrado: ' + msg;
  return msg;
}

// FASE 1: abrir só o TCP, com relógio próprio. Saber que o socket conectou (ou
// não) é o que separa "problema de rede" de "problema de servidor".
function conectarTCP(host, port) {
  return new Promise((resolve, reject) => {
    const socket = net.connect({ host, port });
    let pronto = false;
    socket.setTimeout(TCP_CONNECT_TIMEOUT_MS);
    socket.on('timeout', () => {
      if (pronto) return;
      const e = new Error('tempo esgotado'); e.code = 'ETIMEDOUT';
      socket.destroy(); reject(e);
    });
    // O handler de erro FICA depois de conectar (como no-op): entre o resolve e
    // o ssh2 assumir o socket há uma folga de event loop, e um 'error' sem dono
    // ali derrubaria o processo.
    socket.on('error', (err) => { if (!pronto) reject(err); });
    socket.once('connect', () => { pronto = true; socket.setTimeout(0); resolve(socket); });
  });
}

// A mensagem quando o TCP nem abre: é problema de REDE, e o texto diz o que
// checar — inclusive o clássico do Steam Deck.
function mensagemDeRede(host, port, err) {
  const alvo = `${host.host}:${port}`;
  if (err && err.code === 'ECONNREFUSED') {
    return `Conexão recusada em ${alvo} — o host respondeu, mas não há SSH ouvindo nessa porta (o sshd está ligado?).`;
  }
  if (err && (err.code === 'ENOTFOUND' || err.code === 'EAI_AGAIN')) {
    return `Não encontrei o host "${host.host}" — confira o endereço (o DNS não resolveu).`;
  }
  // ETIMEDOUT / EHOSTUNREACH / ENETUNREACH: o pacote caiu no vazio.
  return `Não consegui abrir a conexão até ${alvo} — o host não respondeu. `
    + 'Verifique se ele está ligado e acessível (IP certo, mesma rede, firewall) e se o SSH está habilitado. '
    + 'No Steam Deck o Wi-Fi costuma dormir: um "ping" no aparelho antes costuma acordá-lo.';
}

// `cred` vem SEMPRE de lib/credenciais.resolver — nunca de host.auth direto.
// Era em `host.auth` que esta função lia, e ela era um dos sete leitores diretos
// espalhados pelo app. Com cofre externo no jogo, um leitor esquecido não
// devolve campo vazio: devolve conexão sem senha.
function buildConnectConfig(host, cred, state, onSaveFingerprint) {
  const cfg = {
    host: host.host,
    port: host.port || 22,
    // O cadastro do host manda; o usuário do cofre só entra quando o host não
    // define — senão editar o host deixaria de ter efeito, sem explicação.
    username: host.username || cred.usuario || '',
    // readyTimeout é setado no connect(), sobre o socket já aberto: cobre só o
    // handshake+auth, não a fase TCP (que tem relógio próprio, com outra mensagem).
    hostHash: 'sha256',
    // TOFU: fixa o fingerprint na 1ª conexão e bloqueia se mudar depois
    hostVerifier(hash) {
      const fp = String(hash);
      if (!host.fingerprint) {
        onSaveFingerprint(fp);
        return true;
      }
      if (host.fingerprint === fp) return true;
      state.mismatch = { expected: host.fingerprint, got: fp };
      return false;
    },
  };
  if (cred.type === 'password') {
    cfg.password = cred.password || '';
    cfg.tryKeyboard = true;
  } else if (cred.type === 'key') {
    // Do cofre a chave vem em memória; do cadastro, de um arquivo no disco.
    cfg.privateKey = cred.privateKey || fs.readFileSync(expandHome(cred.keyPath));
    if (cred.passphrase) cfg.passphrase = cred.passphrase;
  } else {
    if (!process.env.SSH_AUTH_SOCK) {
      throw new Error('Agente SSH indisponível (SSH_AUTH_SOCK não definido). Use chave ou senha, ou inicie o app num terminal com o agente ativo.');
    }
    cfg.agent = process.env.SSH_AUTH_SOCK;
  }
  return cfg;
}

async function connect(host, { onSaveFingerprint, register }) {
  // A credencial é buscada ANTES de abrir o socket. Vinda de cofre, isto é uma
  // chamada de rede que pode falhar — e falhar aqui produz uma mensagem que diz
  // o que houve, em vez de um "falha ao autenticar" que manda o usuário procurar
  // no lugar errado.
  let cred;
  try {
    cred = await credenciais.resolver(host);
  } catch (err) {
    // O CÓDIGO viaja junto do Error. Sem ele, quem recebe só tem texto em
    // português — e a agenda, que precisa decidir entre "tenta de novo" e
    // "desiste desta faixa", ficaria decidindo por substring de mensagem.
    const e = new Error(err.codigo ? `Cofre de credenciais: ${err.message}` : humanizeError(err, host));
    if (err.codigo) e.codigoCofre = err.codigo;
    throw e;
  }
  const state = {};
  let cfg;
  try {
    cfg = buildConnectConfig(host, cred, state, onSaveFingerprint);
  } catch (err) {
    cred.dispose();
    throw new Error(humanizeError(err, host));
  }

  // FASE 1 — TCP. Se nem abrir, a falha é de REDE e a mensagem diz isso; o SSH
  // nem chega a ser tentado.
  const porta = host.port || 22;
  let socket;
  try {
    socket = await conectarTCP(host.host, porta);
  } catch (err) {
    cred.dispose();
    throw new Error(mensagemDeRede(host, porta, err));
  }

  // FASE 2 — SSH sobre o socket JÁ aberto. O readyTimeout agora cobre só o
  // handshake+auth; se estourar, sabemos que o TCP estava bom (tcpOk).
  cfg.sock = socket;
  cfg.readyTimeout = SSH_HANDSHAKE_TIMEOUT_MS;

  return new Promise((resolve, reject) => {
    const conn = new Client();
    // A credencial vive enquanto a conexão viver. `dispose` tira o valor do
    // cadastro de redação; esquecer disto deixaria o segredo protegido para
    // sempre (vazamento de memória) ou desprotegido cedo demais.
    conn.on('close', () => cred.dispose());
    if (register) register(conn);
    let settled = false;
    conn.on('ready', () => {
      settled = true;
      resolve(conn);
    });
    conn.on('error', (err) => {
      if (settled) return;
      settled = true;
      if (state.mismatch) {
        reject(new Error(`A identidade do servidor mudou! Fingerprint esperado ${state.mismatch.expected}, recebido ${state.mismatch.got}. Se a mudança for legítima, use "esquecer fingerprint" no cadastro do host.`));
      } else {
        reject(new Error(humanizeError(err, host, { tcpOk: true })));
      }
    });
    conn.on('close', () => {
      if (!settled) {
        settled = true;
        reject(new Error('Conexão encerrada antes de autenticar.'));
      }
    });
    // Teclado-interativo: a guarda é a CREDENCIAL RESOLVIDA, não o cadastro.
    //
    // Era `host.auth.type === 'password'`, e lia `host.auth.password` direto — o
    // oitavo leitor direto que lib/credenciais.js existe para eliminar. O efeito
    // era exato e silencioso: num host de cofre, `auth.type` é 'cofre', então o
    // respondedor NUNCA era registrado. Só que `cfg.tryKeyboard` era ligado do
    // mesmo jeito (o `cred.type` resolvido é 'password'), então o ssh2 anunciava
    // que sabia responder e depois não respondia.
    //
    // Num servidor com `PasswordAuthentication no` e `KbdInteractiveAuthentication
    // yes` — a configuração de quem usa PAM ou MFA — o ssh2 emitia o evento,
    // ninguém escutava, e a conexão ficava pendurada até o readyTimeout. A
    // mensagem final era "Tempo esgotado ao conectar", que manda investigar rede
    // quando o problema é autenticação.
    //
    // Ou seja: senha salva no cadastro funcionava, e a MESMA senha vinda do cofre
    // não. Exatamente o caso que a integração existe para atender.
    if (cred.type === 'password') {
      conn.on('keyboard-interactive', (n, i, l, prompts, finish) =>
        finish(prompts.map(() => cred.password || '')));
    }
    conn.connect(cfg);
  });
}

// `aoAbrir` recebe o stream assim que ele existe. Sem isso não há como matar o
// comando no servidor: fechar a conexão faz o cliente parar de ouvir, mas o
// processo remoto continua rodando — o agente prometia "Parar imediato" e só
// parava de olhar.
function execCommand(conn, command, { timeoutSec, onData, aoAbrir }) {
  return new Promise((resolve, reject) => {
    conn.exec(command, (err, stream) => {
      if (!err && stream && typeof aoAbrir === 'function') { try { aoAbrir(stream); } catch {} }
      if (err) {
        reject(err);
        return;
      }
      const started = Date.now();
      const written = { out: 0, err: 0 };
      const truncated = { out: false, err: false };
      let timer = null;
      let timedOut = false;
      let done = false;

      const finish = (code, signal) => {
        if (done) return;
        done = true;
        if (timer) clearTimeout(timer);
        conn.removeListener('close', onConnClose);
        resolve({ code, signal: signal || null, durationMs: Date.now() - started, timedOut });
      };
      // conexão caiu ou foi cancelada no meio do comando
      const onConnClose = () => finish(null, null);
      conn.on('close', onConnClose);

      if (timeoutSec > 0) {
        timer = setTimeout(() => {
          timedOut = true;
          // Fechar a conexão só faz o CLIENTE parar de ouvir: o processo segue
          // rodando no servidor. O sinal é o que realmente mata — mesma coisa
          // que o stop() do agente já fazia.
          try { stream.signal('KILL'); } catch {}
          try { stream.close(); } catch {}
          try { conn.end(); } catch {}
        }, timeoutSec * 1000);
      }

      const push = (kind, chunk) => {
        written[kind] += chunk.length;
        if (written[kind] <= MAX_OUTPUT_BYTES) {
          onData(kind, chunk.toString('utf8'));
        } else if (!truncated[kind]) {
          truncated[kind] = true;
          onData(kind, `\n[saída truncada em ${Math.round(MAX_OUTPUT_BYTES / 1024)} KB]\n`);
        }
      };
      stream.on('data', (c) => push('out', c));
      stream.stderr.on('data', (c) => push('err', c));
      stream.on('close', (code, signal) => finish(code === undefined ? null : code, signal));
    });
  });
}

async function pool(items, limit, worker) {
  const results = new Array(items.length);
  let next = 0;
  const lanes = Array.from({ length: Math.min(Math.max(1, limit), items.length) }, async () => {
    while (next < items.length) {
      const idx = next++;
      results[idx] = await worker(items[idx]);
    }
  });
  await Promise.all(lanes);
  return results;
}

function startRun({ perHost, playbookName, options, saveData }) {
  const run = {
    id: crypto.randomUUID(),
    status: 'executando',
    startedAt: Date.now(),
    canceled: false,
    events: [],
    subscribers: new Set(),
    active: new Map(),
  };
  runs.set(run.id, run);
  for (const [id, r] of runs) {
    if (runs.size <= MAX_RUNS) break;
    if (r.status !== 'executando') runs.delete(id);
  }

  emit(run, {
    type: 'run-start',
    playbookName: playbookName || null,
    hostCount: perHost.length,
    // com @cada a contagem pode variar por host — soma o total real
    commandCount: perHost.reduce((sum, p) => sum + p.items.length, 0),
    options,
  });

  const worker = async ({ host, items }) => {
    emit(run, {
      type: 'host-start',
      host: host.id,
      name: host.name,
      address: `${host.username}@${host.host}:${host.port || 22}`,
    });
    if (run.canceled) {
      emit(run, { type: 'host-end', host: host.id, status: 'cancelado' });
      return 'cancelado';
    }

    let conn;
    try {
      conn = await connect(host, {
        register: (c) => run.active.set(host.id, c),
        onSaveFingerprint: (fp) => {
          host.fingerprint = fp;
          // Espelhado: o pin vai para o cofre de confiança; o objeto renasce
          // a cada renovação e perderia o aprendizado.
          if (!dadosDeCofre.fixarFingerprint(host.id, fp, host)) saveData();
          emit(run, { type: 'notice', host: host.id, message: `Fingerprint do servidor registrado (primeira conexão): ${fp.slice(0, 20)}…` });
        },
      });
    } catch (err) {
      run.active.delete(host.id);
      const status = run.canceled ? 'cancelado' : 'erro';
      emit(run, { type: 'host-end', host: host.id, status, error: run.canceled ? undefined : err.message });
      return status;
    }

    let status = 'ok';
    for (let i = 0; i < items.length; i++) {
      if (run.canceled) {
        status = 'cancelado';
        break;
      }
      const command = items[i].resolved;
      emit(run, { type: 'cmd-start', host: host.id, index: i, command });
      let result;
      try {
        result = await execCommand(conn, command, {
          timeoutSec: options.timeoutSec,
          onData: (kind, text) => emit(run, { type: 'data', host: host.id, index: i, kind, text }),
        });
      } catch (err) {
        emit(run, { type: 'cmd-end', host: host.id, index: i, code: null, signal: null, durationMs: 0, timedOut: false, error: err.message });
        status = run.canceled ? 'cancelado' : 'erro';
        break;
      }
      emit(run, {
        type: 'cmd-end',
        host: host.id,
        index: i,
        code: result.code,
        signal: result.signal,
        durationMs: result.durationMs,
        timedOut: result.timedOut,
      });
      if (run.canceled && result.code === null) {
        status = 'cancelado';
        break;
      }
      if (result.timedOut) {
        status = 'erro';
        emit(run, { type: 'notice', host: host.id, message: 'Timeout — a conexão com este host foi encerrada.' });
        break;
      }
      if (result.code !== 0) {
        status = 'erro';
        if (options.stopOnError) {
          if (i < items.length - 1) {
            emit(run, { type: 'notice', host: host.id, message: 'Execução interrompida neste host (parar no primeiro erro).' });
          }
          break;
        }
      }
    }

    run.active.delete(host.id);
    try { conn.end(); } catch {}
    emit(run, { type: 'host-end', host: host.id, status });
    return status;
  };

  (async () => {
    const results = await pool(perHost, options.sequential ? 1 : PARALLEL_HOSTS, worker);
    const counts = { ok: 0, erro: 0, cancelado: 0 };
    for (const s of results) counts[s] = (counts[s] || 0) + 1;
    run.status = run.canceled ? 'cancelado' : counts.erro ? 'erro' : 'ok';
    emit(run, { type: 'run-end', status: run.status, durationMs: Date.now() - run.startedAt, counts });
    setTimeout(() => {
      for (const res of run.subscribers) {
        try { res.end(); } catch {}
      }
      run.subscribers.clear();
    }, 200);
    // Execução terminada continua na memória até 30 runs mais novos existirem —
    // com TODA a saída dentro (até 4000 eventos por run). Dez minutos é folga de
    // sobra para quem quiser reabrir o painel; depois disso fica só o rodapé,
    // que é o que o replay tardio precisa.
    const podar = setTimeout(() => { run.events = run.events.slice(-50); }, 10 * 60 * 1000);
    if (typeof podar.unref === 'function') podar.unref();
  })();

  return run;
}

function cancel(run) {
  if (run.status !== 'executando') return false;
  run.canceled = true;
  emit(run, { type: 'notice', message: 'Cancelamento solicitado — encerrando conexões…' });
  for (const conn of run.active.values()) {
    try { conn.end(); } catch {}
  }
  return true;
}

async function testHost(host, { saveData }) {
  let conn;
  try {
    conn = await connect(host, {
      onSaveFingerprint: (fp) => {
        host.fingerprint = fp;
        if (!dadosDeCofre.fixarFingerprint(host.id, fp, host)) saveData();
      },
    });
  } catch (err) {
    return { ok: false, error: err.message };
  }
  try {
    const result = await execCommand(conn, 'echo conexao-ok', { timeoutSec: 10, onData: () => {} });
    if (result.code === 0) return { ok: true, fingerprint: host.fingerprint || null };
    return { ok: false, error: `Comando de teste retornou código ${result.code}.` };
  } catch (err) {
    return { ok: false, error: err.message };
  } finally {
    try { conn.end(); } catch {}
  }
}

module.exports = { startRun, cancel, testHost, getRun, subscribe, unsubscribe, connect, execCommand,
  // expostos para teste da lógica de mensagem por fase
  _humanizeError: humanizeError, _mensagemDeRede: mensagemDeRede,
  _TCP_CONNECT_TIMEOUT_MS: TCP_CONNECT_TIMEOUT_MS, _SSH_HANDSHAKE_TIMEOUT_MS: SSH_HANDSHAKE_TIMEOUT_MS };
