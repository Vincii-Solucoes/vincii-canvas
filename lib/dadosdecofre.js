...
async function buscarDeVerdade(cofre, e) {
  try {
    ...
    // Zerada ANTES da tentativa, e não dentro do sucesso: um cofre que deixe de
    // declarar `capacidades.listar` não pode ficar com a marca presa travando
    // o recolhimento de confiança para sempre.
    e.segredosIncompletos = false;
    if (adapt && adapt.capacidades.listar && typeof adapt.listar === 'function') {
      try {
        ...
        let cursor;
        do {
          // Só a PRIMEIRA página é retentada: é a que decide se a listagem
          // existe. Retentar cada página multiplicaria o pior caso.
          const chamada = () => adapt.listar(cfgCompleta, { limite: 200, cursor });
          const rl = cursor ? await chamada() : await credenciais.comRecuo(chamada);
        ...
        // A entrada segue "sem erro" porque os SISTEMAS vieram — mas a lista de
        // segredos NÃO veio, e é dela que sai a conta de hosts vivos. Sem esta
        // marca, um 502 só na listagem fazia o recolhimento abaixo concluir
        // "ninguém mais existe" e apagar TODOS os pins TOFU dos espelhados.
        e.segredosIncompletos = true;
      }
    }
...
function limparConfiancaOrfa() {
  ...
  for (const cofre of credenciais.listaDeCofres()) {
    ...
    const e = cache.get(cofre.apelido);
    if (!e || !e.buscadoEm || e.erro || e.segredosTruncados || e.segredosIncompletos) return;
  }
  const vivos = new Set();
