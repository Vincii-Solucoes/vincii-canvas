...
function criarTradutor({ log = () => {} } = {}) {
  ...
  function doCliente(pdu) {
    ...
    // O IronRDP manda a senha mas não pede logon automático. Ligar o bit aqui,
    // antes de assinar e cifrar, é o que faz o servidor usar a credencial em
    // vez de mostrar a tela de login. Escreve no próprio buffer (ver
    // rdp-autologon.js).
    if (!st.trocaFeita && flags && (flags.flags & q.SEC_INFO_PKT)) {
      const r = autologon.marcar(d.carga);
      if (r) log(`Client Info: ${r.marcado
  ...
  function doServidor(pdu) {
    ...
    if (pdu.length > 9 && pdu[7] === 0x7f && pdu[8] === 0x66) {
      ...
      if (g.metodo !== 0) {
        ...
        // Anunciamos só 128 bits; um servidor que responde 40 ou 56 está
        // escolhendo algo que não oferecemos — ou alguém mexeu no caminho.
        if (g.metodo !== cripto.METODO_128) {
          throw new Error(`O servidor escolheu criptografia fraca (método 0x${g.metodo.toString(16)}); só aceitamos RC4 de 128 bits.`);
        }
    ...
    if (!conferirMac(d.carga.slice(4, 12), claro, 'slow-path', flags.flags & q.SEC_SECURE_CHECKSUM)) {
      throw new Error('Assinatura inválida num PDU do servidor — tráfego adulterado ou fora de sincronia.');
    }
  ...
  function desenvelopeFastpath(pdu) {
    ...
    if (!conferirMac(assinaturaRecebida, claro, 'fast-path', pdu[0] & q.FASTPATH_SECURE_CHECKSUM)) {
      throw new Error('Assinatura inválida num quadro de tela — tráfego adulterado ou fora de sincronia.');
    }
